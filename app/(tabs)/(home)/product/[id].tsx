...
/** Vertical space so scroll content clears the fixed Add to cart / Buy now bar (padding + buttons + border). */
const SCROLL_BOTTOM_FOR_ACTION_BAR = 80;
...
export default function ProductDetailScreen() {
  ...
  const { hasAddress, hasPaymentMethod, hydrated: accountHydrated } = useAccount();
  const showBuyNow = accountHydrated && hasAddress && hasPaymentMethod;
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]} edges={[]}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingBottom: SCROLL_BOTTOM_FOR_ACTION_BAR }}>
        <Image
      ...
      <View style={[styles.bar, { backgroundColor: c.card, borderTopColor: c.border }]}>
        <Pressable
          ...
          style={[styles.addBtn, showBuyNow ? styles.addBtnSplit : styles.addBtnFull, { backgroundColor: c.accent }]}>
        ...
        {showBuyNow ? (
          <Pressable
            ...
            onPress={() => {
              addToCart(product.id);
              router.push({ pathname: '/(tabs)/cart', params: { startReview: '1' } });
            }}>
