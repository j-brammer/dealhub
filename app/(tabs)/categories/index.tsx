  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]} edges={['top']}>
      <View style={[styles.header, { paddingHorizontal: 16 }]}>
        ...
        <Text style={[styles.sub, { color: c.muted }]}>US departments — live eBay Browse results</Text>
      </View>
      {categoriesError ? (
      ...
      {categoriesLoading ? (
        ...
        <ScrollView contentContainerStyle={styles.grid} showsVerticalScrollIndicator={false}>
          {categories.map((cat) => (
            <Link key={cat.id} href={{ pathname: '/categories/[id]', params: { id: cat.slug } }} asChild>
              <Pressable
                style={({ pressed }) => [
                  styles.card,
